...
public class AClientShould {
	...
	public void enableToAppendAKeyWithAValue() {
		...
		assertEquals("Can't append a key with a value",
				Constants.PROGRAMM_NAME + "(integer) " + value1.length(),
				mClient.append(key, value1));
		assertEquals("Can't append a key with a value",
				Constants.PROGRAMM_NAME + value1, mClient.get(key));
		
		assertEquals("Can't append a key with a value",
				Constants.PROGRAMM_NAME + "(integer) " + (value1.length() + value2.length()),
				mClient.append(key, value2));
		assertEquals("Can't append a key with a value",
				Constants.PROGRAMM_NAME + value1 + value2, mClient.get(key));
		mClient.delete(key);
	}
